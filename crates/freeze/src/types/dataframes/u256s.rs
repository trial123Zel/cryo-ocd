        ...
        match dtype {
            U256Type::Binary => {
                ...
                match column_encoding {
                    ColumnEncoding::Hex => Ok(Series::new(name.into(), converted.to_vec_hex())),
                    ColumnEncoding::Binary => Ok(Series::new(name.into(), converted)),
                }
            }
            U256Type::String => {
                let converted: Vec<String> = self.iter().map(|v| v.to_string()).collect();
                Ok(Series::new(name.into(), converted))
            }
            U256Type::F32 => {
